...
There are concerns about normality and constant variance of the groups in our analysis. We can try using a non-parametric alternative to one-way ANOVA -  **Kruskal-Wallis** rank sum test, which can be used when ANOVA assumptions are not met.
...
Again, we got a p-value (0.4322), which is significantly larger than 0.05. There's not enough evidence to reject the null hypothesis.