...
```{r}
uberLM_ <- lm(Tip ~ Total_minus_tip + Distance + Time, data=uberML)

summary(uberLM_)
```

The adjusted R^2 slightly increased to almost 6% which is still super low. Good to know that all `Total_minus_tip`, `Distance`, and `Time` are valuable at 5% alpha. The `weekday` variable adds no explanatory power to this multiple linear regression model. Later, we can try categorizing the starting time of the trip variable (`trip_start`) to see if the time of the day makes any difference in the context of tipping.
...
tippedLogitM <- glm(tipped ~ Total_minus_tip + Distance + Time + week_day, data = uberML, family = 'binomial')

summary(tippedLogitM)
```

Not impressive by any means. The only parameter that is statistically significant at 5% alpha is `week_daySun`, at 10% alpha - both `Total_minus_tip` and `Time` are statistically significant. The logistic regression coefficients give the change in the log odds of the outcome for a one unit increase in the predictor variable.

- For every one unit change in `Time` (measured in $ here), the log odds of being tipped (versus non-tipped) increases by 0.159.
- The indicator variables for `week_day` have a slightly different interpretation. For example, making Sunday trip, versus Friday trip, changes the log odds of receiving a tip by -0.51.
...
Here, instead of log odds we get the 'normal' odds like 4 to 1 or 1 to 1. So we can say for a one dollar increase in `Time` earnings, we expect to see about 17% increase in the odds of being tipped (odds are 1.17 to 1). This 17% of increase does not depend on the value that `Time` is held at. 
...
uberML_h2o[,4] = as.factor(uberML_h2o[,4])
...
We ended up with the XGBoost (Extreme Gradient Boosting) model here. It showed pretty bad performance, though. The AUC parameter (area under the curve) is only 54%, which is only 4% higher than the benchmark. We should experiment with XGBoost by tweaking its parameters.
