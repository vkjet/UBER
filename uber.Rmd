...
uberML <- uber_trips_clean %>%
  mutate(date = format(date_time, '%Y-%m-%d'), trip_start = format(date_time, '%H:%M:%S'), tipped = ifelse(is.na(Tip), 'no', 'yes')) %>%
  select(7:32) %>%
  select(tipped, Tip, date, week_day, Total, trip_start, everything()) %>%
  select(1:23) 
...
```{r}
aML <- h2o.automl(8:9, 1, train1, max_runtime_secs = 30, seed = 1234)
